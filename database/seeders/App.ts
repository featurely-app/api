...
export default class AppSeeder extends BaseSeeder {
	public async run() {
		...
		await this.client.transaction(async (trx) => {
			...
			}).client(trx).with('owner', 1, (factory) => {
				factory.merge({
			...
			const project = await tenant
				...
				.create({
					name: 'AdonisJS v5',
					description: 'Project covers the tasks to be accomplished in order to release the final stable version of AdonisJS v5',
			...
			await project
				...
				.create({
					...
					description: await readFile(join(__dirname, '..', 'fixtures', 'env-validations.md'), 'utf-8'),
				})
			...
			await project
				...
				.create({
					...
					description: await readFile(join(__dirname, '..', 'fixtures', 'laravel-collections.md'), 'utf-8'),
				})
		})
	}
}
